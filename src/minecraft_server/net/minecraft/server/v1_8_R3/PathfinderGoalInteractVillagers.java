            ...
            if (this.e == 0)
            {
                ...
                for (int i = 0; i < inventorysubcontainer.getSize(); ++i)
                {
                    ...
                    if (itemstack1 != null)
                    {
                        ...
                        entityitem.motX = (double)(-MathHelper.sin(f1 / 180.0F * (float)Math.PI) * MathHelper.cos(f2 / 180.0F * (float)Math.PI) * fx);
                        entityitem.motZ = (double)(MathHelper.cos(f1 / 180.0F * (float)Math.PI) * MathHelper.cos(f2 / 180.0F * (float)Math.PI) * fx);
                        entityitem.motY = (double)(-MathHelper.sin(f2 / 180.0F * (float)Math.PI) * fx + 0.1F);
                        entityitem.p();
