...
public abstract class EntityHuman extends EntityLiving
{
    ...
    public EntityItem a(ItemStack p_a_1_, boolean p_a_2_, boolean p_a_3_)
    {
        ...
        else
        {
            ...
            {
                float f2 = 0.3F;
                entityitem.motX = (double)(-MathHelper.sin(this.yaw / 180.0F * (float)Math.PI) * MathHelper.cos(this.pitch / 180.0F * (float)Math.PI) * f2);
                entityitem.motZ = (double)(MathHelper.cos(this.yaw / 180.0F * (float)Math.PI) * MathHelper.cos(this.pitch / 180.0F * (float)Math.PI) * f2);
                entityitem.motY = (double)(-MathHelper.sin(this.pitch / 180.0F * (float)Math.PI) * f2 + 0.1F);
        ...
        if (p_attack_1_.aD() && !p_attack_1_.l(this))
        {
            ...
            if (f > 0.0F || f1 > 0.0F)
            {
                ...
                if (flag2)
                {
                    if (i > 0)
                    {
                        p_attack_1_.g((double)(-MathHelper.sin(this.yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F), 0.1D, (double)(MathHelper.cos(this.yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                        this.motX *= 0.6D;
                        this.motZ *= 0.6D;
