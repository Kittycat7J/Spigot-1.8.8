package net.minecraft.server.v1_8_R3;

public class EntitySquid extends EntityWaterAnimal
{
    public float a;
    public float b;
    public float c;
    public float bk;
    public float bl;
    public float bm;
    public float bn;
    public float bo;
    private float bp;
    private float bq;
    private float br;
    private float bs;
    private float bt;
    private float bu;

    public EntitySquid(World p_i161_1_)
    {
        super(p_i161_1_);
        this.setSize(0.95F, 0.95F);
        this.random.setSeed((long)(1 + this.getId()));
        this.bq = 1.0F / (this.random.nextFloat() + 1.0F) * 0.2F;
        this.goalSelector.a(0, new EntitySquid.PathfinderGoalSquid(this));
    }

    protected void initAttributes()
    {
        super.initAttributes();
        this.getAttributeInstance(GenericAttributes.maxHealth).setValue(10.0D);
    }

    public float getHeadHeight()
    {
        return this.length * 0.5F;
    }

    protected String z()
    {
        return null;
    }

    protected String bo()
    {
        return null;
    }

    protected String bp()
    {
        return null;
    }

    protected float bB()
    {
        return 0.4F;
    }

    protected Item getLoot()
    {
        return null;
    }

    protected boolean s_()
    {
        return false;
    }

    protected void dropDeathLoot(boolean p_dropDeathLoot_1_, int p_dropDeathLoot_2_)
    {
        int i = this.random.nextInt(3 + p_dropDeathLoot_2_) + 1;

        for (int j = 0; j < i; ++j)
        {
            this.a(new ItemStack(Items.DYE, 1, EnumColor.BLACK.getInvColorIndex()), 0.0F);
        }
    }

    public void m()
    {
        super.m();
        this.b = this.a;
        this.bk = this.c;
        this.bm = this.bl;
        this.bo = this.bn;
        this.bl += this.bq;

        if ((double)this.bl > (Math.PI * 2D))
        {
            if (this.world.isClientSide)
            {
                this.bl = ((float)Math.PI * 2F);
            }
            else
            {
                this.bl = (float)((double)this.bl - (Math.PI * 2D));

                if (this.random.nextInt(10) == 0)
                {
                    this.bq = 1.0F / (this.random.nextFloat() + 1.0F) * 0.2F;
                }

                this.world.broadcastEntityEffect(this, (byte)19);
            }
        }

        if (this.inWater)
        {
            if (this.bl < (float)Math.PI)
            {
                float f = this.bl / (float)Math.PI;
                this.bn = MathHelper.sin(f * f * (float)Math.PI) * (float)Math.PI * 0.25F;

                if ((double)f > 0.75D)
                {
                    this.bp = 1.0F;
                    this.br = 1.0F;
                }
                else
                {
                    this.br *= 0.8F;
                }
            }
            else
            {
                this.bn = 0.0F;
                this.bp *= 0.9F;
                this.br *= 0.99F;
            }

            if (!this.world.isClientSide)
            {
                this.motX = (double)(this.bs * this.bp);
                this.motY = (double)(this.bt * this.bp);
                this.motZ = (double)(this.bu * this.bp);
            }

            float f1 = MathHelper.sqrt(this.motX * this.motX + this.motZ * this.motZ);
            this.aI += (-((float)MathHelper.b(this.motX, this.motZ)) * 180.0F / (float)Math.PI - this.aI) * 0.1F;
            this.yaw = this.aI;
            this.c = (float)((double)this.c + Math.PI * (double)this.br * 1.5D);
            this.a += (-((float)MathHelper.b((double)f1, this.motY)) * 180.0F / (float)Math.PI - this.a) * 0.1F;
        }
        else
        {
            this.bn = MathHelper.e(MathHelper.sin(this.bl)) * (float)Math.PI * 0.25F;

            if (!this.world.isClientSide)
            {
                this.motX = 0.0D;
                this.motY -= 0.08D;
                this.motY *= 0.9800000190734863D;
                this.motZ = 0.0D;
            }

            this.a = (float)((double)this.a + (double)(-90.0F - this.a) * 0.02D);
        }
    }

    public void g(float p_g_1_, float p_g_2_)
    {
        this.move(this.motX, this.motY, this.motZ);
    }

    public boolean bR()
    {
        return this.locY > 45.0D && this.locY < (double)this.world.F() && super.bR();
    }

    public void b(float p_b_1_, float p_b_2_, float p_b_3_)
    {
        this.bs = p_b_1_;
        this.bt = p_b_2_;
        this.bu = p_b_3_;
    }

    public boolean n()
    {
        return this.bs != 0.0F || this.bt != 0.0F || this.bu != 0.0F;
    }

    static class PathfinderGoalSquid extends PathfinderGoal
    {
        private EntitySquid a;

        public PathfinderGoalSquid(EntitySquid p_i88_1_)
        {
            this.a = p_i88_1_;
        }

        public boolean a()
        {
            return true;
        }

        public void e()
        {
            int i = this.a.bh();

            if (i > 100)
            {
                this.a.b(0.0F, 0.0F, 0.0F);
            }
            else if (this.a.bc().nextInt(50) == 0 || !this.a.inWater || !this.a.n())
            {
                float f = this.a.bc().nextFloat() * (float)Math.PI * 2.0F;
                float f1 = MathHelper.cos(f) * 0.2F;
                float f2 = -0.1F + this.a.bc().nextFloat() * 0.2F;
                float f3 = MathHelper.sin(f) * 0.2F;
                this.a.b(f1, f2, f3);
            }
        }
    }
}
